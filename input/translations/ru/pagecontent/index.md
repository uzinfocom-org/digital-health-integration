...
`MAJOR` и `MINOR` следуют за [UZ Core](https://dhp.uz/fhir/core/). Выпуск `0.7.x` этого руководства собран на основе UZ Core `0.7.x` и согласован с ним - поэтому первый выпуск этого руководства имеет номер `0.7.0`, а не `0.1.0`. `PATCH` за UZ Core не следует: это руководство может выпустить патч самостоятельно, а патч UZ Core не требует выпуска здесь. Точную версию, от которой зависит руководство, смотрите в таблице зависимостей ниже.
