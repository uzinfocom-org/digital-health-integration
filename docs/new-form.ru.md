...
Руководство по созданию новой клинической формы (документа) в этом IG на примере уже готовых форм 011 (гемодиализ), 066 и 066-1 (статистические карты выписки) и 130 (результаты лабораторных исследований). Если существующие формы расходятся между собой, ориентируйтесь на самую свежую по git-истории: конвенции со временем уточняются, а старые примеры не всегда переделываются задним числом.
...
- `Provenance` берите по профилю `UZCoreProvenance` (как в 130): `target = Reference(Bundle/example-form-0XX-name)`, агент - `author` из `$provenance-participant-type`, подпись - `signature` с `type[nationalType]` из `$signature-type-cs`, `who`, `when`, `sigFormat`, `data`. Старые примеры (011/066) используют базовый `Provenance` с `attester` - для новых форм так не делайте.
